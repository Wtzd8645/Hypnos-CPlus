    ...
    receiveBuf(new char[maxPacketBytes])
{
    ...
    {
        Kernel::LogError("[UdpListener] Create listen socket failed. ErrorCode: %d", errno);
        ::exit(EXIT_FAILURE);
    }
    ...
    {
        Kernel::LogError("[UdpListener] Bind adderss to listen socket failed. ErrorCode: %d", errno);
        ::exit(EXIT_FAILURE);
    }
    ...
    {
        Kernel::LogError("[UdpListener] Get flags from listen socket failed. ErrorCode: %d", errno);
        ::exit(EXIT_FAILURE);
    }
    ...
    {
        Kernel::LogError("[UdpListener] Set the non-blocking flag to the listen socket failed. ErrorCode: %d", errno);
        ::exit(EXIT_FAILURE);
    }
    ...
    if (epfd == INVALID_FD)
    {
        Kernel::LogError("[UdpListener] Create epoll from the listen socket failed. ErrorCode: %d", errno);
        ::exit(EXIT_FAILURE);
    }
    ...
    {
        Kernel::LogError("[UdpListener] Register the listen socket epoll event failed. ErrorCode: %d", errno);
        ::exit(EXIT_FAILURE);
    }
    ...
    Kernel::Log("[UdpListener] Create the listen socket success. Bind address: %s", ip);
}
...
inline void UdpListener::Listen()
{
    ...
    Kernel::Log("[UdpListener] The server socket is listening.");
}
...
void UdpListener::ProcessEvents()
{
    ...
    while (true)
    {
        ...
        if (evtNum < 0)
        {
            ...
            Kernel::Log("[UdpListener] Wait for epoll events failed. ErrorCode: %d", errorCode);
...
inline void UdpListener::Receive()
{
    ...
    while (true)
    {
        ...
        if (transferredBytes < 0)
        {
            ...
            Kernel::Log("[UdpListener] Receive from the listen socket failed. ErrorCode: %d", errorCode);
            break;
        }
        ...
        // {
        //     Kernel::LogError("[UdpListener] Create resquest failed.");
        //}
    }
}
        ...
        for (auto respIt = consumerResponses->begin(); respIt != consumerResponses->end(); ++respIt)
        {
            ...
            for (auto connIt = resp->connIds.begin(); connIt != resp->connIds.end(); ++connIt)
            {
                ...
                while (sentBytes < packetBytes) // TODO: Is UDP need to do while send?
                {
                    ...
                    {
                        Kernel::LogError("[UdpListener] Send failed. ErrorCode: %d", errno);
                        break;
                    }
                    sentBytes += result;
                }
