...
void GameDataManager::CreateMmap(const char* dataFilePath)
{
...
int32 GameDataManager::GetKeyTableOffset(const char* tableName)
{
    const int32 NameOffsetField = 4;
    const int32 TableOffsetField = 8;
    const int32 NextOffsetField = 20;

    uint32 hash = BkdrHash(reinterpret_cast<const uint8*>(tableName), strlen(reinterpret_cast<const char*>(tableName)));
...
inline string GameDataManager::GetString(int32 offset)
{
    ...
    char* strPtr = reinterpret_cast<char*>(dataMmapPtr + offset);
